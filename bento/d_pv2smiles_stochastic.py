...
@torch.no_grad()
def generate_with_property_nobeam(model, properties, n_sample, prop_mask, stochastic=True):
    device = model.device
    ...
    n_batch = 10
    prop = properties.unsqueeze(0).repeat(n_sample // n_batch, 1)
...
def generate_with_property(model, properties, n_sample, prop_mask, k=2, stochastic=True):
    ...
    with open('./normalize.pkl', 'rb') as w:
        norm = pickle.load(w)
    property_mean, property_std = norm
    property_mean = property_mean.to(device)
    property_std = property_std.to(device)
    properties = (properties - property_mean) / property_std
